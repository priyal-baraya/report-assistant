...
def ask():
    ...
    # Smart prompt based on report type
    if report_type == "trip":
        prompt = f"""
Today is {today_str}.
You are given a trip report for employees from {start_date} to {end_date}.

Each employee has a field called 'totalTraveledDistance' which is a number (in kilometers) representing how far they traveled.

1. Compare these distances numerically.
2. Do not list every employee unless the user explicitly asks.
3. Just say the employee name and the distance for the person who traveled the most, in a clean sentence like:
   "Jay Asti traveled the most last month, covering 0.08 km."
